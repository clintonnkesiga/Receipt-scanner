...
- ~~Duplicate detection~~ ✅ — primary match on the receipt's unique **fiscal/transaction number** (`fiscal_id`), immune to OCR date/total wobble. Extracted from labels ("Fiscal Doc No / CashSale / Receipt / Invoice No / Verification Code") **and** via an OCR-tolerant year-prefixed pattern (`20\d{2}-\d{4,}`, e.g. `2026-1550307`) that survives garbled labels. Fallback to merchant + total within ±3 days. Detected duplicates **block saving** (`POST /receipts` returns 409) unless the user ticks "Save anyway" (`force: true`).
